      ...
      doContinue() {
        ...
        for (var questionIndex = 0; questionIndex < this.solutioneering101Quiz.questions.length; questionIndex++) {
          ...
          if (userAnswer === correctAnswer) {
            this.updateSolutioneering101QuizQuestionIsCorrectState({ questionIndex: questionIndex, isCorrect: true });
          } else {
            this.updateSolutioneering101QuizQuestionIsCorrectState({ questionIndex: questionIndex, isCorrect: false });
            allQuestionsCorrect = false;
          }
        }

        if (allQuestionsCorrect) {
          this.updateSolutioneering101QuizCompletedStatus(true);
          this.$router.push({ name: 'dashboard' });
        } else {
          this.errorMessage = 'Please correct your answers and select Continue';
        }
      },
      ...mapActions('onboarding', ['updateSolutioneering101QuizQuestionIsCorrectState', 'updateSolutioneering101QuizCompletedStatus']),
    },
    computed: {
